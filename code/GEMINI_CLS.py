...
compounds=[]
print("")
if os.path.exists(f'results/{MODEL_NAME}/submission_EN.tsv') and split=="test":
    computed=pd.read_csv(f'results/{MODEL_NAME}/submission_EN.tsv', delimiter='\t',encoding='utf-8')
...
elif os.path.exists(f'results/{MODEL_NAME}/submission_XE.tsv') and split=="xeval":
    computed=pd.read_csv(f'results/{MODEL_NAME}/submission_XE.tsv', delimiter='\t',encoding='utf-8')
    computed_compounds=computed['compound'].values.tolist()
    computed_predictions=computed['expected_order'].values.tolist()
...
for compound in coompounds_list:
    ...
    if split=="test":
        df_save.to_csv(f'results/{MODEL_NAME}/submission_EN.tsv', sep='\t',index=False, header=True)
    else:
        df_save.to_csv(f'results/{MODEL_NAME}/submission_XE.tsv', sep='\t',index=False, header=True)